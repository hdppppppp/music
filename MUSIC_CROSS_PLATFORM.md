...
- 账号登录、注册和账号级数据隔离
- 酷我主源搜索、队列播放、歌词、收藏、最近播放和离线下载（服务端仍保留腾讯/网易/酷我多音源适配）
...
Android 提供 10、20、30、45、60、90 分钟及 1-1440 分钟自定义时长；Windows 提供 5、10、15、30、60、90 分钟及同范围自定义时长。剩余时间显示在播放页和设置页；重新选择时长会从当前时刻重新计时，选择“关闭”立即取消。计时状态由 Android 播放服务或 Windows 客户端进程持有，应用/播放服务被系统销毁后不会假装恢复旧倒计时，重新打开后可重新设置。
...
Windows 的 FFmpeg 播放链路覆盖常见 MP3、M4A/AAC、WAV、FLAC、OGG/Opus 容器；腾讯 NAC 私有格式暂不保证可解码，界面会提示选择可用档位。Windows 暂不实现 AI 图片工作台、悟空 IM、单曲倒带日记、系统托盘之外的设备协同控制；这些不属于本轮音乐功能对标范围。

传输加密方面：服务端已具备握手与 AEAD 中间件（协议 v2 绑定设备号），Android（`ANDROID_ID`）与
Windows（`MachineGuid`）已提供设备号来源，客户端尚未发出加密头之前，明文请求对服务端完全透明。
